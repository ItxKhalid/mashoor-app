...
class _SearchResultWidgetState extends State<SearchResultWidget> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GetBuilder<MySearchController>(builder: (searchController) {
        bool _isNull = true;
        int _length = 0;
      ...
      Expanded(child: NotificationListener(
        onNotification: (scrollNotification) {
          if (scrollNotification is ScrollEndNotification) {
            Get.find<MySearchController>().setRestaurant(_tabController.index == 1);
            Get.find<MySearchController>().searchData(widget.searchText);
          }
          return false;
        },
        child: TabBarView(
