...
import '../data/model/response/city_list_by_cat_model.dart';
import '../data/model/response/seasonal_product_model.dart';
...
class CategoryController extends GetxController implements GetxService {
  ...
  CityListByCategoryModel _cityListByCategory;
  ...
  List<Season> _allSeasonalProduct=[];
  bool _isLoading = false;
  int _pageSize;
  int _restPageSize;
  bool _isSearching = false;
  int _subCategoryIndex = 0;
  int _cityListByCategoryIndex = 0;
  String _type = 'all';
  ...
  CityListByCategoryModel get cityListByCategory => _cityListByCategory;
  ...
  List<Season> get allSeasonalProduct => _allSeasonalProduct;
  ...
  Future<void> getSeasonalProduct() async {
    Response response = await categoryRepo.getSeasonalProduct();
    if (response.statusCode == 200) {
      _allSeasonalProduct.clear();
      _allSeasonalProduct.addAll(SeasonalProductModel.fromJson(response.body).season);
      print('..........${response.body}');
      _isLoading = false;
      update();
    } else {
  ...
  Future<String> getCityListByCategory(String categoryID) async {
    _cityListByCategoryIndex = 0;
    // _cityListByCategory = null;
    // _categoryProductList = null;
   //  Response response = await categoryRepo.getCityListByCategory(categoryID);
   ...
   //
   //    // Assuming CityListByCategoryModel.fromJson expects a Map<String, dynamic>
   //    CityListByCategoryModel cityListByCategoryModel = CityListByCategoryModel.fromJson(response.body);
   //
   //    // Add the model to your list
   //    _cityListByCategory.add(cityListByCategoryModel);
   //
   //    print('City List Status: ${_cityListByCategory[0].status}');
   //  }
   //  else {
   //    ApiChecker.checkApi(response);
   //  }
    Response response = await categoryRepo.getCityListByCategory(categoryID);
    if (response.statusCode == 200) {
      _cityListByCategory=CityListByCategoryModel.fromJson(response.body);
      // _cityListByCategory.add(CityListByCategoryModel.fromJson(response.body));
      print("this is city list by me ${_cityListByCategory.city.length}");
      // _cityListByCategory.add(CityListByCategoryModel(city: [City(id: int.parse(categoryID), name: 'all'.tr)]));
      // response.body.forEach((category) => _subCategoryList.add(CategoryModel.fromJson(category)));
      getCategoryProductList(categoryID, 1, 'all', false);
    } else {
      ApiChecker.checkApi(response);
