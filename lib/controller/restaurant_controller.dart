...
import '../data/model/response/all_city_model.dart';
...
class RestaurantController extends GetxController implements GetxService {
  ...
  List<Cities> get allCityList => _allCityList;
  ...
  Future<void> getCityList() async {
    Response response = await restaurantRepo.getCityList();
    if (response.statusCode == 200) {
      _allCityList.clear();
      _allCityList.addAll(AllCityModel.fromJson(response.body).cities);
      print('..........${response.body}');
      _isLoading = false;
      update();
    } else {
